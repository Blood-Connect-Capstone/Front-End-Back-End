  ...
  <div class="container my-5" style="max-width: 800px">
    <div class="profile-card d-flex align-items-center p-4 mb-4">
      <div class="profile-photo me-4">
        <label for="profileImage" style="cursor: pointer">
          <img :src="profile?.photo || defaultPhoto" alt="Profile" class="rounded-circle" width="150" height="150" />
          <span class="visually-hidden">Ganti Foto Profil</span>
          <input id="profileImage" type="file" accept="image/*" @change="onImageChange" style="display: none" />
        </label>
      </div>
      <div>
    ...
    <div class="row g-3">
      <template v-if="hasHistory">
        <div v-for="(item, idx) in donorHistory" :key="idx" class="col-md-4">
          <div class="history-card p-3">
            ...
            <div class="mb-1">
              ...
              <i class="bi bi-geo-alt"></i> {{ item.donor_reservations.donor_locations?.name || item.donor_reservations.blood_requests.location.name }}
            </div>
            <div>
...
const { profile, getDonorHistory, fetchProfile, updateImage } = useProfilePresenter();
...
onMounted(async () => {
  ...
  if (res) {
    donorHistory.value = res;
  } else {
    donorHistory.value = [];
  }

  console.log("Donor History:", donorHistory.value);
});

const totalDonor = computed(() => profile.value?.total_donor || 0);
const totalVolume = computed(() => totalDonor.value * 200);
const hasHistory = computed(() => donorHistory.value.length > 0);
